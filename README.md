...
- プロフィール - 自己紹介やSNS URLの編集、プロフィール画像のアップロード・変更・削除
- コンテンツ管理 - 管理者・メンテナー向けの編集機能
- カテゴリー管理 - 資料/動画/アプリ用カテゴリーの追加・編集・削除
- マスター管理（設計中） - admin / maintainer 向けに、`documents` / `videos` / `categories` / `applications` / `positions` / `position_tags` を `npm run db:types:local` で生成した型定義を参照して一覧表示・レコード詳細参照（member には非表示、`users` は対象外）
- カレンダー - シンラボイベントやシンラボMTGおよび日本の祝祭日を統合したカレンダー
- クイックリンク - Slack・ドライブ・公式サイト・SNS等のシンラボ関連リンクをまとめて表示
...
なお、ローカル開発の環境変数は [dotenvx](https://dotenvx.com) で暗号化管理しています（暗号化済みの `.env.development` をリポジトリに含め、復号鍵のみ各自で保管）。詳細は環境構築手順を参照してください。
