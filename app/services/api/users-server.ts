...
export async function fetchActiveUsers(): Promise<{
  ...
  const { data, error } = await supabase
    .from("users")
    .select(
      "id, display_name, role, bio, avatar_url, profile_image_path, x_url, facebook_url, instagram_url, github_url, portfolio_url, position_tags(positions(id, name, is_leadership, is_deleted))"
    )
    .eq("status", "active")
    .eq("is_deleted", false)
