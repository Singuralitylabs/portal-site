...
| `SLACK_WEBHOOK_URL`     | Slack Incoming Webhook URL。アプリ通知と Wiki 更新通知で共通利用する | アプリケーション通知、Wiki 更新通知ワークフロー（実装予定） | 通知先チャンネルを変更する場合は Slack 側で Webhook を再発行して差し替える |
...
| 通知内容 | 対象ページ、操作種別、Wiki コミット、変更差分、Actions 実行ログへのリンク  |
| 通知先   | `SLACK_WEBHOOK_URL` Secret に設定した Slack Incoming Webhook のチャンネル  |

`gollum` イベントでは対象 Wiki ページのコミット SHA が渡される。ワークフローは Wiki リポジトリ（`portal-site.wiki.git`）を取得し、対象コミットの差分を生成して Slack に通知する。差分が長い場合は Slack のメッセージ上限に収まるよう一部を省略し、詳細確認用に Actions の実行ログを併記する。

### 5.2 失敗時の扱い

| 失敗パターン               | 影響                               | リカバリ手順                                                                                |
| -------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------- |
| `SLACK_WEBHOOK_URL` 未設定 | Slack 通知が送信されない           | GitHub Actions の Secrets に `SLACK_WEBHOOK_URL` を登録し、失敗したワークフローを再実行する |
| Slack Webhook 送信失敗     | Slack 通知が送信されない           | Slack 側の Webhook 設定・チャンネル権限を確認し、ワークフローを再実行する                   |
