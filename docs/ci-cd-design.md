...
差分が長い場合は、ページ単位で見出し抽出を優先する。差分抜粋はページごとに先頭 5 行まで、かつ 100 字を超える場合は先頭 100 字で打ち切る。省略した場合は、詳細確認用に compare revisions へのリンクを併記する。
...
5. `update_summary` に通知へ表示したい更新概要を必要に応じて入力する。未入力時は既定文言を使う。
