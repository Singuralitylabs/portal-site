...
| `default_sort`              | `JSONB`        | NOT NULL, DEFAULT '[]'::jsonb       | 既定ソート設定（例: カラム・昇降順）                    |
...
`allow_list` / `allow_create` / `allow_update` / `allow_delete` は、テーブル単位の上限設定（機能フラグ）として扱う。ロール別の最終的な操作可否は `master_admin_permissions` の `can_*` と組み合わせて判定し、対象操作は `allow_* = true` かつ対象ロールの `can_* = true` の場合のみ許可する。
...
erDiagram
    ...
    master_admin_tables {
      ...
      BOOLEAN allow_list "一覧参照可否"
      BOOLEAN allow_create "追加可否"
      BOOLEAN allow_update "更新可否"
      BOOLEAN allow_delete "削除可否"
      VARCHAR delete_mode "削除方式"
      VARCHAR logical_delete_column "論理削除カラム名"
      VARCHAR physical_delete_phrase "物理削除時の固定入力文言"
      JSONB default_sort "既定ソート設定"
    ...
    master_admin_columns {
      ...
      VARCHAR display_name "画面表示名"
      INTEGER display_order "画面表示順"
      BOOLEAN is_visible "一覧表示可否"
      BOOLEAN is_editable "編集可否"
      BOOLEAN is_searchable "検索対象可否"
      BOOLEAN is_filterable "フィルター対象可否"
      BOOLEAN is_sortable "ソート対象可否"
      BOOLEAN is_required_on_create "追加時必須か"
      BOOLEAN is_required_on_update "更新時必須か"
      VARCHAR input_type "入力方式"
      VARCHAR options_source_type "選択肢ソース"
      JSONB options_static "固定値選択肢"
      VARCHAR ref_schema_name "外部キー参照先スキーマ"
      VARCHAR ref_table_name "外部キー参照先テーブル"
      VARCHAR ref_value_column "外部キー参照先の値カラム"
      VARCHAR ref_label_column "外部キー参照先の表示カラム"
      INTEGER created_by FK "作成ユーザー（users.id）"
      INTEGER updated_by FK "更新ユーザー（users.id）"
      TIMESTAMP created_at "作成日時"
      TIMESTAMP updated_at "更新日時"
    ...
    master_admin_permissions {
      ...
      BOOLEAN can_view_detail "詳細参照可否"
...
- マスター管理画面のテーブル一覧・カラム操作可否は、`master_admin_tables` / `master_admin_columns` / `master_admin_permissions` の設定で判定する
- テーブル単位の `allow_*` は操作可否の上限設定とし、ロール別の `can_*` とAND条件で評価する
...
- `master_admin_tables` / `master_admin_columns` / `master_admin_permissions` を対象にRLSを有効化する
- `admin` のみ設定テーブルの登録・更新・削除を可能とする
- `maintainer` は設定テーブルを参照のみ可能とする（更新系は不可）
- `member` は設定テーブルを参照不可とする
- 設定テーブルの許可設定だけで最終可否を確定せず、最終判定は常に対象テーブルのRLSポリシーで行う
