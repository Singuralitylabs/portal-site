...
マスター管理機能は、専用の設定テーブルを追加せず、既存テーブル定義と `npm run db:types` により生成される `app/types/lib/database.types.ts` を参照して対象テーブル・カラム・型情報を解決します。
設定テーブル方式を用いる拡張設計は [#408](https://github.com/Singuralitylabs/portal-site/issues/408) で別途検討します。
...
### 2.8. マスター管理機能で参照するスキーマ情報

- 専用の設定テーブルは追加しない
- `app/types/lib/database.types.ts` をスキーマ参照の正本として利用する
- 対象テーブル名、カラム名、データ型、NULL可否、リレーションは `db:types` と既存スキーマ定義から解決する
- 一覧表示、入力方式、検索・フィルター条件は、型ベースの共通ルールで決定する
- `db:types` 参照で扱えるのはスキーマ由来の情報までとし、画面表示名、表示順、固定値選択肢、個別のCRUD制御などの補助メタデータは保持しない
- 固定値選択肢やテーブル別・カラム別の補助設定をDBで管理する拡張設計は [#408](https://github.com/Singuralitylabs/portal-site/issues/408) で別途検討する
...
- 初回実装では、`users` を除く既存テーブルのうち、本設計で扱う対象テーブルを一覧参照できることを目的とする
...
- マスター管理画面のテーブル一覧・カラム・型情報は、`app/types/lib/database.types.ts` と既存スキーマ定義を参照して判定する
- RLSポリシーより広い権限をUIやサーバーAPIで許可してはならない
- DBテーブル・カラム・RLSポリシーを変更した場合は、`npm run db:types` を再生成し、一覧表示仕様との整合確認を必須とする
- 固定値選択肢や個別設定をDBで管理する方式は、本設計では採用せず [#408](https://github.com/Singuralitylabs/portal-site/issues/408) で検討する
