...
> タイムスタンプ等の手動クエリパラメータをURLに付与するのではなく、Storageアップロード時に `cacheControl: "0"` を指定したうえで、画像更新時は署名付きURLを再取得すること。ただし、署名付きURLの `iat`/`exp` は秒単位のため、同一パスを同一秒内に再取得すると偶然同一のURL文字列になる場合がある。そのため即時反映が必要な画面（`app/providers/profile-image-provider.tsx`）では、URL文字列の変化だけに頼らず、取得のたびにインクリメントする更新世代（`profileImageVersion`）を保持し、表示側の `key` として使うことで再描画を保証する。
