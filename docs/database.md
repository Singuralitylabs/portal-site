...
8. **会員一覧表示区分情報** (`positions2`テーブル)  
   会員一覧画面における表示区分（代表・副代表・シンラボ管理人・メンバー）を管理します。各ユーザーはいずれか1つの区分に属します。
...
| `positions2_id`      | `INTEGER`      | FK(positions2.id), DEFAULT 9, NOT NULL | 会員一覧表示区分ID（1:代表 / 2:副代表 / 3:シンラボ管理人 / 9:メンバー） |
...
会員一覧画面での表示区分を管理する固定マスターテーブル。各ユーザーは `users.positions2_id` でいずれか1つの区分に紐付けられる。

| カラム名      | データ型      | 制約        | 説明                           |
| ------------- | ------------- | ----------- | ------------------------------ |
| `id`          | `INTEGER`     | PRIMARY KEY | 区分ID（固定値）               |
| `name`        | `VARCHAR(50)` | NOT NULL    | 区分名                         |
| `description` | `TEXT`        |             | 区分の説明文                   |
...
| 2  | 副代表           | シンラボの副代表                  |
| 3  | シンラボ管理人   | シンラボの管理人                  |
| 9  | メンバー         | 一般メンバー（新規ユーザーの既定） |

> id=4〜8は将来の拡張（副管理人等の追加）に予約。
...
erDiagram
    users {
        ...
        INTEGER positions2_id FK "会員一覧表示区分ID（positions2.id）(デフォルト: 9)"
    ...
    positions2 {
        INTEGER id PK "区分ID（固定値: 1/2/3/9）"
        VARCHAR name "区分名 (最大50文字)"
        TEXT description "区分の説明文"
    }
    ...
    users }o--|| positions2 : "N:1 (positions2_id)"
...
### 4.8. positions2 テーブルのRLSポリシー

- 認証済みユーザーは全データを閲覧可能（固定マスターデータのため削除フラグ不要）
- INSERT/UPDATE は管理者のみ可能（データは固定値のため通常変更しない）
- 物理削除は禁止

### 4.9. profile-images Storage のRLSポリシー
