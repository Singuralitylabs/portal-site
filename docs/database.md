...
| `profile_image_path` | `TEXT`    |                  　　　　　　　　　　 |カスタムプロフィール画像のpath                     |　
                   | カスタムプロフィール画像のパス（Supabase Storage） |
...
保存パスを `{auth_id}/profile-image`（拡張子なし）に固定することで、再アップロード時に常に同一オブジェクトを上書きし、不要なファイルの蓄積を防ぐ。
ユーザーが異なる拡張子のファイルをアップロードしても、保存名は `profile-image` で統一されるため不整合は発生しない。

> **キャッシュ対策**: 同一パスへの上書きはブラウザキャッシュにより古い画像が表示され続ける問題が起きやすい。
> 実装時は署名付きURL取得時またはパブリックURL生成時にタイムスタンプをクエリパラメータとして付与すること（例: `...avatar?t=1712345678`）。
