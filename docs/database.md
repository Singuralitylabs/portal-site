...
- 管理者またはメンテナーは全データを登録・更新可能
- 管理者またはメンテナーは論理削除が可能
- カテゴリー削除時は、対象カテゴリーに紐づく既存コンテンツ（documents/videos/applications）を同一カテゴリー種別の「未分類」カテゴリーへ移動してから、対象カテゴリーを論理削除する
- 「未分類」カテゴリーは `category_type` と `name='未分類'` の組み合わせで特定する
- 移動先の「未分類」カテゴリーが存在しない場合は、先に作成してから削除処理を実行する
- 論理削除のみとし、物理削除を防止
