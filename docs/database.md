...
- `users` の参照は `admin` ロールのみに許可し、`maintainer`/`member` は参照不可とする
...
- SQL 実行機能はローカル環境・Vercel環境の双方で提供可能
- SQL はアプリサーバーの専用APIから実行し、ユーザーセッションを引き継いだ状態で RLS を適用する
- service role による RLS バイパスは本機能では使用しない
- 初期導入は `SELECT` のみ許可し、更新系SQLは別途レビュー後に段階導入する
- 実行時は複文禁止、タイムアウト、取得件数上限を適用する
- 実行履歴（実行者・時刻・SQL要約・成否）を監査ログとして記録する
