...
| `is_admin()`           | `BOOLEAN` | 現在のユーザーが `admin` ロールを持つかを判定する。ユーザーの承認・権限変更・利用停止など管理者専用操作の確認に使用。必ず `is_active_user()` と組み合わせて使用すること。                       |

いずれも `SECURITY DEFINER` で定義しており、関数内の `SELECT` はRLSを経由しない。このため `users` テーブル自身のポリシーから `is_admin()` を呼び出しても再帰は発生しない。
