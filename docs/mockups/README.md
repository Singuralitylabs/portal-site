...
## ブラウザ描画 + Playwright で SVG 生成する手順

実装済みの HTML モックをブラウザで描画し、描画結果を SVG 化する PoC パターンです。
...
2. 実行
	...
	```bash
	npm run mockup:svg:playwright
	```

3. 出力先
	- `docs/mockups/svg/login.playwright.svg`
...
- 必要に応じて `MOCKUP_BASE_URL` で対象ホストを変更できます
	...
	```bash
	MOCKUP_BASE_URL="http://127.0.0.1:3001" npm run mockup:svg:playwright
	```

## 方針

- 生成元の正本は `docs/mockups/specs/*.json` とする
- 生成物は `docs/mockups/svg/*.svg` として設計書から参照する
