...
セキュアストアの運用がどうしても難しい場合に限り、受け取った鍵を `.env.keys` としてプロジェクトルートに置く方法もある。この場合 Step 2・Step 3 は不要で、dotenvx がプロジェクトルートの `.env.keys` を自動的に読む。

プロジェクトルートに `.env.keys` を作成し、次の 1 行だけを書く（ターミナルで実行するコマンドではない）。
