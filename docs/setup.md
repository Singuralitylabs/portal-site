...
| Secret              | 用途                                                                                      | 補足                                                                                                                      |
| ------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `SLACK_WEBHOOK_URL` | Wiki 更新通知ワークフロー（実装予定）が参照する Incoming Webhook URL（GitHub Actions 用） | Slack の Incoming Webhook URL を登録する。アプリ通知は実行環境の環境変数（`.env.local` / ホスティング環境変数）で設定する |

Wiki 更新通知ワークフロー（実装予定）は `portal-site.wiki.git` を read 権限で取得する。ワークフロー側で `permissions: contents: read` を明示し、追加の専用トークンは発行しない。
...
2. 作成した `.env.local` の各値を実際のものに差し替える **（各値は参画時に個別共有）**。
   ...
   | `SLACK_WEBHOOK_URL`             | Slack に通知を送るための Incoming Webhook URL（任意設定）。未設定時は通知をスキップして処理継続する。申請・承認イベント等の通知送信先として使用し、GitHub Actions では Wiki 更新通知にも同じ URL を共通利用する                                                                                                                                                                                                             | Slack の Incoming Webhook 設定画面                                                    |
