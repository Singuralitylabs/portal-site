...
10. [カレンダーページ]（#9-カレンダーページ）
    > > > > > > > d436c19 (カレンダー機能についての記述)
...
シンギュラリティ・ラボのイベントおよびMTGカレンダーをポータルサイト内に表示する機能。Googleカレンダーの埋め込み機能を利用し、常に最新のスケジュール情報を提供します。
...
| :----------------- | :------------------------------------------------------------------------------------------------------------------ |
| **ページパス**     | `/calendar`                                                                                                         |
| **埋め込み方式**   | Googleカレンダーの `<iframe>` 埋め込みコードを使用。                                                                |
| **埋め込み対象**   | シンラボイベントカレンダー、シンラボMTGカレンダーなど、指定されたGoogleカレンダー。                                 |
| **認証**           | 認証済みのシンラボメンバーのみがアクセス可能 (`app/(authenticated)/` ルートに配置)。                                |
| **ナビゲーション** | サイドバー（`SideNav`コンポーネント）に「カレンダー」リンクを追加。アイコンは `lucide-react` の `Calendar` を使用。 |

### 9.3 権限に関する注意点（管理者確認事項）

本機能はGoogleカレンダーの公開設定に依存します。

- サイトに埋め込むカレンダーは、**ログイン済みの全メンバーが閲覧できるよう**、Googleカレンダー側で適切な共有設定がされている必要があります。
- 閲覧権限がない場合、カレンダーは正しく表示されず、エラーメッセージが表示されます。
