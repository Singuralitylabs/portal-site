...
3. 新規登録
   ...
   - クリックして新規登録用のモーダルを表示
      ...
      - 表示順の設定：
         ...
         - 選択された位置に基づいて display_order が自動的に計算・設定される
         - 挿入位置以降の既存カテゴリーの display_order は自動的に +1 される
...
5. 削除
   ...
   - 「未分類」カテゴリーが存在しない場合は、先に作成してから削除処理を行う
...
- 導線 > サイドバー（権限別）
  - 一般メンバー（`member`）：`カテゴリー管理` と `管理画面` は非表示
  - コンテンツ管理者（`maintainer`）：`カテゴリー管理` は表示、`管理画面` は非表示
  - 管理者（`admin`）：`カテゴリー管理` と `管理画面` を表示
