...
シンラボ内の各種データを適切に管理する機能を提供する。コンテンツ管理権限（管理者またはメンテナー）を持つユーザーが、資料や動画の追加・編集・削除を行える。
...
1. 承認管理
   ...
   - 管理者にのみ、サイドバーに「ダッシュボード」メニューを追加
     - クリックでメンバー紹介ページ（`/dashboard`）に遷移
...
1. ダッシュボード > 承認管理
   ...
   - カード形式での表示
     ...
     - レスポンシブ対応（画面幅に応じてカード数を調整）
   - 表示条件
     - 承認済み（`status='pending'`）のメンバー
     - 論理削除されていないメンバー
   - 各カードごとに「承認」「否認」「削除」のボタンを表示
     - 「承認」ボタンをクリックして、承認確認用のモーダルを表示
       - 内容を確認して「承認」ボタンをクリックすると、該当ユーザが承認される (status が `active`)
       - 「キャンセル」ボタンをクリックすると、「承認管理」に遷移する
     - 「否認」ボタンをクリックして、否認確認用のモーダルを表示
       - 内容を確認して「否認」ボタンをクリックすると、該当ユーザが否認される (status が `rejected`)
       - 「キャンセル」ボタンをクリックすると、「承認管理」に遷移する
     - 「削除」ボタンをクリックして、削除確認用のモーダルを表示
       - 内容を確認して「削除」ボタンをクリックすると、該当ユーザが論理削除される (is_deleted が `true`)
       - 「キャンセル」ボタンをクリックすると、「承認管理」に遷移する
   - 「メンバー一覧（管理） 」ボタンから、「メンバー一覧（管理） 」画面に遷移する

2. メンバー一覧（管理）

   - 管理者にのみ、「メンバー一覧（管理）」をカード形式での表示
     - 1行3カードのグリッドレイアウト
     - レスポンシブ対応（画面幅に応じてカード数を調整）
   - 表示内容
     - 論理削除されていない、全メンバーのカードを表示
     - ステータスによって、色分けする（承認待ち：黄、承認済み：青、否認：赤）
     - 別途示すカード項目と、最終行にプルダウンで現在のステータス、「更新」ボタンを用意
   - カード操作：
     - 変更したメンバーのカード上でステータスを選択をして「更新」をクリック
       - 内容を確認して「更新」ボタンをクリックすると、該当ユーザのステータスが更新される
       - 「キャンセル」ボタンをクリックすると、「メンバー一覧（管理）」の一覧に戻る
     - 「更新」と並んで「削除」ボタンで削除
       - 内容を確認して「削除」ボタンをクリックすると、該当ユーザが論理削除される (is_deleted が `true`)
       - 「キャンセル」ボタンをクリックすると、「メンバー一覧（管理）」の一覧に戻る
...
メンバー一覧（管理） / カード上には以下の情報を表示：

- プロフィール画像、表示名
- メールアドレス
- 更新日
- ステータス（初期値は現在のステータス、プルダウン可能）
