...
- **観点**
  - **認証制御**: 未認証ユーザーが保護ページにアクセスした場合、ログインへ誘導される
  - **認可制御**: ロール（admin / maintainer / member）に応じて許可/拒否が正しく判定される
  - **承認ステータス制御**: 未承認（pending）・拒否（rejected）のユーザーが適切なページへ誘導される
  - **データアクセス**: DB側のRLSにより意図しないデータ参照ができない（自動テストは未実装）
  - **セッション管理**: 期限切れ等でログイン状態が無効な場合に適切に再ログインへ誘導される
