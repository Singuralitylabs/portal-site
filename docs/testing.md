...
マスター管理機能は対象テーブルが広く、認可や意図しないデータ露出の影響が大きいため、設計段階で以下の観点を定義する。
...
- **ユニットテスト対象**
  ...
  - ロールと `is_deleted` などの共通ルールに基づいて、一覧表示対象のレコードを正しく判定できること
...
- **実行タイミング**
  ...
  - 主要表示フローは実装PRで手動確認またはE2E確認の対象とする
