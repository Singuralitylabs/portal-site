...
- **実装方針（提案）**
  - **ユニット**: `app/services/auth/*` の関数を Jest で検証。例: `checkAdminPermissions`, `checkContentPermissions`, `getServerAuth` など。
  - **統合**: Next.js `middleware.ts` と `AuthLayout` を対象に、Jestで `NextRequest` をモックして遷移・リダイレクトを検証。
  - **E2E**: Playwright で `storageState` を用いた role matrix を最小本数で実行。
  - **CI**: `test:security` を追加し、PRでは `--selectProjects` などで軽量なユニットのみ実行。統合/E2Eは別ワークフローで ワークフロー手動実行 とリリース前運用に限定。
