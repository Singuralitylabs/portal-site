...
| Supabase DB Types Consistency ([.github/workflows/db-types.yml](../.github/workflows/db-types.yml))                    | DB 型定義の整合性監視        | 型生成 + 差分チェック         | `push` / `pull_request`（`supabase/**`, `app/types/lib/database.types.ts` 等）、`workflow_dispatch`                         |
