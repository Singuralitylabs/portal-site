...
- **対象領域（最小範囲の例）**
  ...
  - ユーザー管理（登録・承認）
  - 参照系マスタ（カテゴリ等）
  - `database.types.ts` を参照した表示項目・外部キー参照表示の解決
...
管理者・メンテナー向け画面では、`member` が対象画面へアクセスできないこと、対象外テーブルが参照できないこと、UI制御だけでなくサーバーAPI側でも同じ権限制御が強制されることを確認する。
...
- **観点**
  ...
  - **スキーマ参照**: `is_deleted` を持つテーブルでも、型定義参照に基づく共通表示処理が `is_deleted` で表示対象レコードをフィルタリングしないことを確認する
