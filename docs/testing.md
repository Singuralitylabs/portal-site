...
- **観点**
  ...
  - **コード整形**: Prettier 未適用のフォーマット崩れを検知して失敗させる（`format-check.yml`）
...
| ---------------------------------------------------------------------------------------------------------------------- | ---------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Build Test ([.github/workflows/build.yml](../.github/workflows/build.yml))                                             | 本番相当のビルド成立性を検証 | 依存関係インストール + ビルド                                         | `push` / `pull_request`（`app/**`, `middleware.ts`）、`workflow_dispatch`                                                                    |
...
| Jest Unit Tests ([.github/workflows/test.yml](../.github/workflows/test.yml))                                          | ユニットテスト実行           | ユニットテスト                                                        | `push` / `pull_request`（`app/**`, `tests/**`, `middleware.ts`, `jest.config.js`, `package.json`, `package-lock.json`）、`workflow_dispatch` |
| Check console.log and debugger ([.github/workflows/check_console_log.yml](../.github/workflows/check_console_log.yml)) | デバッグ用出力の混入を防止   | console/debugger 検査（`app/` と `middleware.ts`）                    | `push` / `pull_request`（`app/**`, `middleware.ts`, `scripts/lint-logs.cjs`, `package.json`, `package-lock.json`）、`workflow_dispatch`      |
| Supabase DB Types Consistency ([.github/workflows/db-types.yml](../.github/workflows/db-types.yml))                    | DB 型定義の整合性監視        | 型生成 + 差分チェック（必須設定未定義時はエラー終了し対応方法を表示） | `push` / `pull_request`（`supabase/**`, `app/types/lib/database.types.ts` 等）、`workflow_dispatch`                                          |
| Format Check ([.github/workflows/format-check.yml](../.github/workflows/format-check.yml))                             | コード整形の統一性を担保     | Prettier フォーマットチェック（`format:check`）                       | `push` / `pull_request`（全ファイル対象）、`workflow_dispatch`                                                                               |
...
- **ESLint**: 静的解析
- **Prettier**: コード整形（`format:check` / `format`）
- **カスタムスクリプト**: デバッグ出力検査（`lint:logs`）
- **Supabase CLI**: 型生成・スキーマ整合性確認（CI で自動実行）
...
### 4.4 Supabase DB Types チェックの前提設定

- `.github/workflows/db-types.yml` を実行するために、GitHub Actions 側で以下の設定が必要。
- **Secret**: `SUPABASE_ACCESS_TOKEN`（Supabase CLI で利用する Personal Access Token）
- **Variable**: `SUPABASE_PROJECT_ID`（対象 Supabase プロジェクトの Project ID）
...
| `tests/api/notifications/slack-notification.test.ts` | `app/api/notifications/slack/route.ts`                                                                                                                                                                                                     | `POST`                                                                                                                                                                                                                                                                                                                                                                                           | Slack 通知 API の正常系（pending ユーザーの送信/スキップ）と未認証 401・認可 403・入力検証 400 を検証する。 |
| `tests/api/calendar.test.ts`                         | `app/api/calendar/calendar-server.ts`, `app/api/calendar/events/route.ts`                                                                                                                                                                  | `fetchCalendarEvents`, `GET`                                                                                                                                                                                                                                                                                                                                                                     | Google Calendar 連携の正常取得、未認証 401、inactive 403、認証失敗時のエラー応答を検証する。                |
| `tests/middleware.test.ts`                           | `middleware.ts`                                                                                                                                                                                                                            | `middleware`                                                                                                                                                                                                                                                                                                                                                                                     | API 未認証時の 401 JSON、ページ未認証時の /login リダイレクト、pending ユーザーの API 通過を検証する。      |
| `tests/utils/middleware-path.test.ts`                | `app/utils/middleware-path.ts`                                                                                                                                                                                                             | `shouldSkipMiddleware`, `isApiPath`, `isPublicPageRoute`, `isPublicApiRoute`                                                                                                                                                                                                                                                                                                                     | `/api` を一律スキップしないこと、公開 API ホワイトリストが空であることを検証する。                          |
...
| `tests/services/api/trash-client.test.ts`            | `app/services/api/trash-client.ts`                                                                                                                                                                                                         | `restoreDocument`, `restoreVideo`, `restoreApplication`, `restoreCategory`                                                                                                                                                                                                                                                                                                                       | 復活処理の正常系/異常系、display_order 末尾計算、削除済み条件での取得・更新を検証する。                     |
| `tests/services/api/trash-server.test.ts`            | `app/services/api/trash-server.ts`                                                                                                                                                                                                         | `fetchDeletedDocuments`, `fetchDeletedVideos`, `fetchDeletedApplications`, `fetchDeletedCategories`                                                                                                                                                                                                                                                                                              | 削除済み一覧取得の正常系/異常系、および updated_at 降順の並び順仕様を検証する。                             |
