...
| Check console.log and debugger ([.github/workflows/check_console_log.yml](../.github/workflows/check_console_log.yml)) | デバッグ用出力の混入を防止   | console/debugger 検査                                                 | `push` / `pull_request`（`app/**`, `middleware.ts`, `scripts/lint-logs.cjs`, `package.json`, `package-lock.json`）、`workflow_dispatch`      |
