...
| 複数ステップで整合性を保つ更新処理         | 途中失敗時の部分成功が起きやすく、戻り値と実データの不整合を検知する必要がある         | `supabase-client.test.ts`（categories-client）   |
...
- **観点**
  ...
  - 失敗返却時に、実データ状態が契約と整合すること（ロールバック/再試行の成立）
- **対象領域（最小範囲の例）**
  - コンテンツ取得（一覧・詳細）
  - コンテンツ管理（登録・更新・削除）
  - ユーザー管理（登録・承認）
...
- ただし、返り値だけでは正しさを判定できない整合性ロジック（表示順再採番・シフト・ロールバック等）は、更新対象や更新値の検証を許容する
...
| `tests/services/api/supabase-client.test.ts`         | `app/services/api/supabase-client.ts`, `app/services/api/applications-client.ts`, `app/services/api/categories-client.ts`, `app/services/api/documents-client.ts`, `app/services/api/videos-client.ts`, `app/services/api/users-client.ts` | `createClientSupabaseClient`, `getApplicationsByCategory`, `deleteApplication`, `registerApplication`, `updateApplication`, `registerCategory`, `updateCategory`, `deleteCategory`, `getDocumentsByCategory`, `deleteDocument`, `registerDocument`, `updateDocument`, `getVideosByCategory`, `deleteVideo`, `registerVideo`, `updateVideo`, `addNewUser`, `fetchUserRoleById`, `fetchUserStatusById`, `fetchUserIdByAuthId`, `approveUser`, `rejectUser` | クライアント側 CRUD・カテゴリ整合性ロジック・ユーザー操作について、返り値・エラーハンドリング・副作用（再採番/ロールバック等）中心に検証する。 |
