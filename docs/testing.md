...
マスター管理機能は対象テーブルが広く、認可・データ破壊の影響が大きいため、設計段階で以下の観点を定義する。

- **ユニットテスト対象**
  - `app/types/lib/database.types.ts` の参照結果から表示項目・入力方式・検索/フィルター条件を解決する処理
  - `db:types` の型定義が対象DBテーブル・カラム名と整合していること
  - 外部キー参照と型ベースの入力方式が画面入力仕様と整合していること
  - DB enum または外部キーとして表現されていない平文 `string` カラムに、固定値プルダウン前提のUIを適用していないこと
  - RLSポリシーより広い操作権限をUIやサーバーAPIで許可していないこと
  - 物理削除の多段確認条件判定
...
- **セキュリティテスト対象**
  ...
  - RLSで許可されないデータを一覧結果で扱えないこと

- **画面/操作テスト対象**
  - 左ペインメニューからマスター管理画面へ遷移できること
  - テーブル選択、一覧表示、フィルター、ソートが機能すること
  - レコード検索が文字列系カラムの共通ルールに基づく検索対象に対してのみ機能すること
  - DB enum または外部キーとして表現された項目がプルダウンで表示され、自由入力できないこと
  - 論理削除可能なテーブルでは論理削除が既定動作になること
  - 物理削除は2段階以上の確認を通過しないと実行できないこと

- **レスポンシブ確認対象**
  - モバイル幅でテーブル選択がドロワーまたはセレクトに切り替わること
  - 一覧、フィルター、追加/編集フォーム、削除確認が画面幅内で操作できること

- **実行タイミング**
  - 認可・テーブル定義処理はユニットテストを追加する
  - 主要操作フローは実装PRで手動確認またはE2E確認の対象とする
  - テーブル追加・カラム追加・カラム変更時は、`db:types` の更新とテストを更新する
