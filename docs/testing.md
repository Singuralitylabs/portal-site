...
| `tests/api/profile-image.test.ts` | `app/api/profile/image/route.ts`, `app/services/api/users-server.ts` | `POST`, `updateUserProfileImagePathInServer`, `updateUserProfileImageUrlInServer` | プロフィール画像 API の正常系、入力検証、ストレージ更新失敗時のエラーハンドリングを検証する。 |
