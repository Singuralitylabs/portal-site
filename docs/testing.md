...
| `tests/services/api/supabase-client.test.ts`         | `app/services/api/supabase-client.ts`, `app/services/api/categories-client.ts`, `app/services/api/documents-client.ts`, `app/services/api/users-client.ts`                                                                               | `createClientSupabaseClient`, `registerCategory`, `updateCategory`, `deleteCategory`, `getDocumentsByCategory`, `deleteDocument`, `registerDocument`, `updateDocument`, `addNewUser`, `fetchUserRoleById`, `fetchUserStatusById`, `fetchUserIdByAuthId`, `approveUser`, `rejectUser`                                                                                                                                                         | document 代表の CRUD、カテゴリ整合性ロジック、ユーザー操作の代表ケースについて、返り値・エラーハンドリング・主要な副作用を中心に検証する。     |
