...
- **ユニットテスト対象**
  ...
  - `master_admin_tables` / `master_admin_columns` / `master_admin_permissions` の設定がDBテーブル・カラム名と整合していること
  - 固定値選択肢と外部キー参照設定が画面入力仕様と整合していること
  - RLSポリシーより広い操作権限を設定テーブルで許可していないこと
  - 物理削除の多段確認条件判定
...
- **実行タイミング**
  ...
  - テーブル追加・カラム追加・カラム変更時は、設定テーブル定義とテストを更新する
