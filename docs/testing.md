...
| Wiki Slack Notification（実装予定: `.github/workflows/wiki-slack-notification.yml`）                                   | Wiki 更新のチーム連携        | Wiki 更新内容を 1 件の Slack 通知にまとめ、差分抜粋と compare リンクを通知 | Wiki 更新（`gollum`）、`workflow_dispatch`（更新ページ・更新概要を入力）                                                    |
