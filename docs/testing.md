...
3. [テスト対象と観点](#3-テスト対象と観点)
   ...
   - [3.7 マスター管理機能のテスト観点](#37-マスター管理機能のテスト観点)
...
マスター管理機能は対象テーブルが広く、認可・データ破壊・CSV取り込みの影響が大きいため、設計段階で以下の観点を定義する。
...
- **ユニットテスト対象**
  ...
  - 検索対象カラム、ページサイズ、一括更新対象カラム、表示可能カラムの設定解決
  - 一括更新時の対象レコード・対象カラム・更新値のバリデーション
  - CSVインポートのパース、カラム対応、許可値チェック、エラー行抽出
  - CSVエクスポートの対象カラム・フィルター条件適用
  - 物理削除の多段確認条件判定

- **セキュリティテスト対象**
  - `member` がマスター管理画面へアクセスできないこと
  - `maintainer` がマスター管理画面で `users` を参照できないこと
  - `admin` のみマスター管理画面から `users` を参照できること
  - UI制御だけでなく、サーバーAPI側でも同じ権限制御が強制されること
  - RLSで許可されないデータを一覧・CSVエクスポート・CSVインポート結果で扱えないこと

- **画面/操作テスト対象**
  - 左ペインメニューからマスター管理画面へ遷移できること
  - テーブル選択、一覧表示、フィルター、列表示、ページネーション、ソートが機能すること
  - レコード検索が設定済みの検索対象カラムに対してのみ機能すること
  - ページネーションでページ番号、表示件数、総件数が正しく表示されること
  - カラムの表示/非表示切替が設定で許可されたカラムに限定されること
  - 複数レコードを一括選択し、選択件数・対象レコード・差分を確認できること
  - 一括更新で更新対象カラム、更新値、影響件数、失敗理由を確認できること
  - 使用可能な値が決まっている項目がプルダウンで表示され、自由入力できないこと
  - 論理削除可能なテーブルでは論理削除が既定動作になること
  - 物理削除は2段階以上の確認を通過しないと実行できないこと

- **CSVテスト対象**
  - フィルター適用後のデータのみCSVエクスポートできること
  - CSVインポート前にプレビューと影響件数が表示されること
  - 不正な許可値、必須項目不足、型不一致、外部キー不整合がある行は取り込まれないこと
  - エラー行がある場合、行番号・項目・理由が表示されること

- **レスポンシブ確認対象**
  - モバイル幅でテーブル選択がドロワーまたはセレクトに切り替わること
  - 一覧、フィルター、追加/編集フォーム、削除確認、CSV操作が画面幅内で操作できること

- **実行タイミング**
  - 認可・テーブル定義・CSV処理はユニットテストを追加する
  - 主要操作フローは実装PRで手動確認またはE2E確認の対象とする
  - テーブル追加・カラム追加・カラム変更時は、`npm run db:types` 後にマスター管理用設定とテストを更新する
