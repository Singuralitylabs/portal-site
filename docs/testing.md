...
- **ユニットテスト対象**
  ...
  - `npm run db:types:local` で生成される型定義差分は 4.1 の Supabase DB Types Consistency CI で検知すること
  - 外部キー参照表示が画面表示仕様と整合していること
  - `is_deleted` を持つ対象テーブルでも、`is_deleted` に基づく表示対象レコードのフィルタリングを行わないこと
  - RLSポリシーより広い参照権限をUIやサーバーAPIで許可していないこと

- **セキュリティテスト対象**
  - `member` がマスター管理画面へアクセスできないこと
  - 対象外テーブルがマスター管理画面から参照できないこと
...
- **画面/操作テスト対象**
  ...
  - テーブル選択と一覧表示が機能すること
  - 論理削除済みレコードも一覧表示とレコード詳細参照の対象に含まれること
  - 行選択時にレコード詳細を参照できること
  - 外部キー項目が参照先の情報とともに表示できること
  - ゴミ箱機能で論理削除したレコードを復活できること
  - ゴミ箱機能で復活した後、対象レコードの内容が更新され、画面表示と整合すること

- **レスポンシブ確認対象**
  - モバイル幅でテーブル選択がドロワーまたはセレクトに切り替わること
  - 一覧とレコード詳細表示が画面幅内で操作できること

- **実行タイミング**
  - 認可・テーブル定義処理はユニットテストを追加する
  - 主要操作フローは実装PRで手動確認またはE2E確認の対象とする
  - 検証環境では、Supabase 管理画面の Table Editor の表示と、マスター管理画面およびゴミ箱画面の表示・更新結果が一致することを確認する
  - テーブル追加・カラム追加・カラム変更時は、`npm run db:types:local` を実行して型定義を更新し、テストを更新する
