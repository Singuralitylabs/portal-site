...
ローカル環境で supabase 接続に使う URL・キーは、dotenvx で暗号化された `.env.development` で管理します（設定手順は [環境構築手順](../docs/setup.md) を参照）。復号鍵を受け取れば、以下を個別に設定する必要はありません。

対象の環境変数（値は `.env.development` に暗号化済み）:
...
1. `npm run db:types:local`を実行して型定義を更新
2. 必要に応じてテーブル定義を修正
