...
--
-- 【注意】本ファイルを単独で再実行しないこと（00_users_policies.sql と同様の理由）
-- 24行目の active_users_can_read_all は issue #424 で廃止され、
-- 02_restrict_active_users_read_all.sql の admins_can_read_all_users（管理者限定）に
-- 差し替えられている。permissive ポリシーはOR結合されるため、本ファイルを単独で
-- 再実行すると条件の緩い旧定義が復活し、列レベルPII露出の修正が無効化される。
-- 再実行する場合は必ず 02_restrict_active_users_read_all.sql も続けて実行すること。
